        ...
        db.query('SELECT COUNT(*) as total FROM parking_slots WHERE status = ?', ['occupied'], (err, r4) => {
          ...
          db.query('SELECT COUNT(*) as total FROM bookings WHERE status = ?', ['active'], (err, r5) => {
            ...
            db.query('SELECT COUNT(*) as total FROM bookings', (err, r6) => {
              ...
              db.query(
                'SELECT COALESCE(SUM(total_fee),0) as revenue FROM bookings WHERE payment_status = ?',
                ['paid'],
                (err, r7) => {
                  if (err) return res.status(500).json({ error: err.message });
...
router.get('/chart/bookings', verifyToken, (req, res) => {
  const sql = `
    SELECT
...
router.post('/slots', verifyToken, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ message: '✅ Slot added!', id: result.insertId });
    }
  );
});
...
// ── Update slot status ──
router.put('/slots/:id', verifyToken, (req, res) => {
